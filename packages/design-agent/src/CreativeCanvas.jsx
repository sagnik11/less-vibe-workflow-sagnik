...
export default function CreativeCanvas({
  ...
  // Theme handling: self-managed state with localStorage persistence.
  // Falls back to forcedTheme prop, then saved preference, then "dark".
  const [internalTheme, setInternalTheme] = useState(() => {
    if (forcedSetTheme && forcedTheme) return forcedTheme;
    if (typeof window !== "undefined") {
      return localStorage.getItem("design-agent-theme") || forcedTheme || "dark";
    }
    return forcedTheme || "dark";
  });
  const resolvedTheme = (forcedSetTheme && forcedTheme) ? forcedTheme : internalTheme;
  const setTheme = forcedSetTheme || ((t) => {
    setInternalTheme(t);
    if (typeof window !== "undefined") localStorage.setItem("design-agent-theme", t);
  });
  ...
  const fetchBalance = async () => {
    try {
      const { data } = await axios.get(`${API}/account/balance`, { headers: getHeaders() });
  ...
  const sendMessage = async (textOverride = null, skillOverride = null, attachmentsOverride = null) => {
    ...
    try {
      ...
      // Strip heavy event data from snapshot to keep payload small
      const lightMessages = updatedMessages.map(m => ({
        role: m.role,
        content: m.content || "",
        ...(m.timestamp ? { timestamp: m.timestamp } : {}),
        ...(m.skill_name ? { skill_name: m.skill_name } : {}),
      }));
      ...
      let payload = {
        message: typed,
        model: "gpt-5-mini",
        messages_snapshot: lightMessages,
      ...
      setMessages(prev => {
        const arr = [...prev];
        if (aIdx >= 0) arr[aIdx] = { ...arr[aIdx], content: `❌ ${detail}` };
        return arr;
      });
    } finally {
      setBusy(false);
  ...
  return (
    <div className={`h-dvh w-full text-sm flex flex-col bg-bg-page text-primary-text overflow-hidden ${resolvedTheme === "dark" ? "dark" : ""}`} style={{ fontFamily: "'Inter', sans-serif" }}>
            ...
            <div className="flex items-center gap-2">
              {!inEmbedMode && (
                <div className="flex items-center gap-2 h-8 border border-divider rounded bg-bg-page/30 overflow-hidden px-2">
                  ...
                  >
                    {userBalanceLabel ?? `$ ${balance ?? user?.balance ?? "0.00"}`}
                  </span>
                </div>
              )}
                ...
                >
                  <div className="px-4 py-3 border-b border-divider flex flex-col">
                    ...
                    <div className="mt-2 text-[13px] font-bold text-primary">
                      {userBalanceLabel ?? `$ ${balance ?? user?.balance ?? "0.00"}`} <span className="font-normal text-secondary-text">available</span>
                    </div>
                  </div>
                    ...
                    <div className={`max-w-[90%] space-y-2 ${msg.role === "user" ? "text-right" : "text-left"}`}>
                      <div className="relative">
                        <div className={`px-3 py-2 text-[13px] leading-relaxed break-words relative
                          ${msg.role === "user" ? "bg-primary text-white rounded-md rounded-tr-none shadow-sm" : "text-primary-text bg-bg-page rounded-md rounded-tl-none shadow-sm border border-divider"}`}>
                          
                          {msg.content ? (
                            msg.role === "assistant" ? (
                              <div className="prose dark:prose-invert max-w-none prose-p:leading-relaxed prose-pre:bg-black/30 prose-headings:text-primary-text prose-strong:text-primary-text prose-li:text-primary-text text-primary-text">
                                <ReactMarkdown
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="prose prose-invert max-w-none text-white prose-p:leading-relaxed prose-p:text-white prose-headings:text-white prose-strong:text-white prose-li:text-white">
                                  <ReactMarkdown
                                    remarkPlugins={[remarkGfm]}
                                    components={markdownComponents}
                                  >
                                ...
                                {msg.attachments && msg.attachments.length > 0 && (
                                  <div className="flex flex-col gap-2 mt-2 w-full">
                                    {msg.attachments.map(att => (
                                      <div key={att.asset_label} className="relative w-full rounded border border-white/30 overflow-hidden shadow-sm bg-white/10">
                                        {att.kind === "image" && (
